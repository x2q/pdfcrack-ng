...
static void
print_and_clean_wall_time(const char *str, unsigned int nrprocessed,
...
static void
pdf_128b_bench(int nrOfThreads) {
	clock_t startTime, endTime;
	...
	initPDFCrack(&e, NULL, true, NULL, Generative, NULL, charset, 0, 4, true, 0, 1, 1);
	...
	initPDFCrack(&e, NULL, false, NULL, Generative, NULL, charset, 0, 4, true, 0, 1, 1);
	...
	initPDFCrack(&e,password, false, NULL, Generative, NULL, charset, 0, 4,true, 0, 1, 1);
	...
	if (nrOfThreads > 0){
		...
		//		initPDFCrack(&e, NULL, true, NULL, Generative, NULL, charset, 0, 4, true, 0);
		//		gettimeofday(&wallTimeStart, NULL);
		//		runCrackRev3();
		//		gettimeofday(&wallTimeEnd, NULL);
		//		print_and_clean_wall_time("PDF (128, user, non threaded):", getNrProcessed(), wallTimeStart, wallTimeEnd);

		initPDFCrack(&e, NULL, true, NULL, Generative, NULL, charset, 0, 5, true, nrOfThreads, 1, 1);
		gettimeofday(&wallTimeStart, NULL);
		runCrackMultiThreads();
		gettimeofday(&wallTimeEnd, NULL);
		char buf[32];
		sprintf(buf, "PDF (128, user, %i threads):", nrOfThreads);
...
pdf_40b_bench(void) {
	...
	initPDFCrack(&e, NULL, true, NULL, Generative, NULL, charset, 0, 5, true, 1, 1, 1);
	...
	initPDFCrack(&e, NULL, false, NULL, Generative, NULL, charset, 0, 5, true, 1, 1, 1);
	...
	initPDFCrack(&e, password, false, NULL, Generative, NULL, charset,0, 5,true, 1, 1, 1);
